...
const CreateProduct = () => {
  ...
  const handleCreateProduct = async () => {
    ...
    for (let i = 0; i < filesUpload.length; i++) {
      data.append('files', filesUpload[i]) // nối nhiều file vào một biến
    }

    const filesResult = await dispatch(uploadImages(data)).unwrap() // upload lên toàn bộ file và trả về nhiều url
    ...
    filesResult.forEach((url) => {
      dispatch(uploadImage({ productId: createProductResult.id, url: url })) // nhét url vào product
    })
    setImagesUpload([])
    setProduct({
      name: '',
  ...
  const categories = useSelector((state) => state.category.data) //get data từ bên redux
